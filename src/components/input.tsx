...
CHANGES
  ...
  - created CustomizableInput & InputRenderer component
  - export InputRenderer instead of Input
...
function CustomizableInput({
  ...
  return (
    <div className={cn("ep:relative ep:flex ep:w-fit ep:gap-2")}>
      <Input
        data-slot="input"
        className={cn(
          {
...
function InputRenderer({
  ...
  right,
  ...props
}: React.ComponentProps<typeof CustomizableInput>) {
  return !right && !left ? (
    <Input {...props} />
  ) : (
    <CustomizableInput left={left} right={right} {...props} />
  );
}
